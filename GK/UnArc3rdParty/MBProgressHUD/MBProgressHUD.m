...
//wsong updated begin
-(UIView*)maskView:(CGRect)rect
{
    if (!maskView) {
        maskView = [[UIView alloc] initWithFrame:rect];
        maskView.backgroundColor = [UIColor blackColor];
//        [self addSubview:maskView];
        [self insertSubview:maskView belowSubview:indicator];
...
- (void)updateIndicators {
    ...
    if (mode == MBProgressHUDModeDeterminate) {
        ...
        NSLog(@"MBProgressHUDModeCustomView");
...
- (id)initToView:(UIView *)view {
    ...
    self.maskView = [self maskView:rect];
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.maskView) {
        ...
        CGContextSetShadowWithColor (context, myShadowOffset, 3, myColor);
        // Your drawing code here
