...
-(void)startDownloadingFromURL {
    ...
    item.url = url;
    item.handler = self;
    if ([HJMOHandler QUEUE].count >= QUEUE_MAX_NUM) item.status = FALSE;
    else item.status = TRUE;
    [HJMOHandler addQueueItem:item];
   // [item release];
...
-(void)startDownloadingFromURL:(NSURL *)iurl{
	//NSLog(@"HJMOHandler starting download for %@",self);
	...
	[urlConn release];
	if (urlConn==nil) {
		NSLog(@"HJMOHandler nil URLConnection for %@",iurl);
		state=stateFailed;
	} else {
		state=stateLoading;
		//TODO if app is showing a network activity monitor in the status bar, here is where a call needs to be 
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection {
    ...
    [HJMOHandler removeQueueItem:url];
    [HJMOHandler goon];
}
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    [HJMOHandler removeQueueItem:url];
    [HJMOHandler goon];
  //  QueueItem *item = [HJMOHandler getFreeItem];
  //  item.status = TRUE;
}
...
+(void)removeQueueItem:(NSURL*)url
{
    NSInteger max = [HJMOHandler QUEUE].count;
    QueueItem *item;
    
    for (int i = 0; i < max; i++) {
        item = [[HJMOHandler QUEUE] objectAtIndex:i];
        
        if (item.url == url) {
            [[HJMOHandler QUEUE] removeObjectAtIndex:i];
            break;
        }
    }
}
...
+(void)addQueueItem:(QueueItem*)item
{
   // NSInteger max = [HJMOHandler QUEUE].count;
    ...
    [[HJMOHandler QUEUE] addObject:item];
}
...
+(void)goon
{
    ...
    [item.handler startDownloadingFromURL:item.url];
    //return item;
}
