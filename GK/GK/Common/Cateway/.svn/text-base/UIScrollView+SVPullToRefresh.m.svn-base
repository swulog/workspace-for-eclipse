...
@property (nonatomic,assign) BOOL frameInited;
...
static char UIScrollViewPullToRefreshView,UIScrollViewPullToLoadMoreView;

@implementation UIScrollView (SVPullToRefresh)

@dynamic pullToRefreshView, showsPullToRefresh, pullToLoadMoreView,showsPullToLoadMore;
...
- (void)addPullToRefreshWithActionHandler:(void (^)(void))actionHandler position:(SVPullToRefreshPosition)position {
    ...
    //if(!self.pullToRefreshView)
    {
        ...
        {
            view.originalTopInset = self.contentInset.bottom;
            view.originalBottomInset = self.contentInset.bottom;
            view.position = position;
...
-(void)removePullToLoadMoreView
{
    if (self.pullToLoadMoreView && self.pullToLoadMoreView.superview) {
        [self.pullToLoadMoreView removeFromSuperview];
    }
...
- (void)setPullToLoadMoreView:(SVPullToRefreshView *)pullToRefreshView {
    [self willChangeValueForKey:@"SVPullToLoadMoreView"];
    objc_setAssociatedObject(self, &UIScrollViewPullToLoadMoreView,
                             pullToRefreshView,
                             OBJC_ASSOCIATION_ASSIGN);
    [self didChangeValueForKey:@"SVPullToLoadMoreView"];
}

- (SVPullToRefreshView *)pullToLoadMoreView {
...
- (void)setShowsPullToLoadMore:(BOOL)showsPullToLoadMore {
    ...
    if(!showsPullToLoadMore) {
        ...
        if (!self.pullToLoadMoreView.isObserving) {
            ...
            CGFloat yOrigin;
            switch (self.pullToLoadMoreView.position) {
                case SVPullToRefreshPositionTop:
                    yOrigin = -SVPullToRefreshViewHeight;
                    break;
                case SVPullToRefreshPositionBottom:
            ...
            self.pullToLoadMoreView.frame = CGRectMake(0, yOrigin, self.bounds.size.width, SVPullToRefreshViewHeight);
        }
    }
}
...
- (id)initWithFrame:(CGRect)frame {
    if(self = [super initWithFrame:frame]) {
        ...
        self.textColor =  colorWithUtfStr(WSRefreshTableTextColor);  // [UIColor darkGrayColor];
        self.autoresizingMask = UIViewAutoresizingFlexibleWidth;
        self.state = SVPullToRefreshStateStopped;
        self.showsDateLabel = NO;
        
        self.titles = [NSMutableArray arrayWithObjects:
                       frame.origin.y<0?NSLocalizedString(@"Pull down to refresh...",):NSLocalizedString(@"Pull up to load more...",),
                       frame.origin.y<0?NSLocalizedString(@"Release to refresh...",):NSLocalizedString(@"Release to load more...",),
...
//    }
    ...
    self.arrow.hidden = hasCustomView || !WSRefreshTableArrowEnabeld;
    ...
    if(hasCustomView) {
        ...
        labelX = (self.bounds.size.width - 84 )/ 2;
#else
        CGFloat labelX = (self.bounds.size.width - titleSize.width ) /2;
#endif
...
#if 0//wsong
            float subX = labelX - 10;
#else
            float subX = (self.bounds.size.width - subtitleSize.width ) /2;

#endif
            self.subtitleLabel.frame = CGRectIntegral(CGRectMake(subX, titleY + titleSize.height + marginY, subtitleSize.width, subtitleSize.height));
        ...
        [self.titleLabel.layer removeAllAnimations]; //wsong
        [self.subtitleLabel.layer removeAllAnimations]; //wsong
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
  //  NSLog(@"scrollViewDidScroll : %f",contentOffset.y);


//    if (!self.frameInited) {
//        self.frameInited = TRUE;
//        self.initFrame = self.scrollView.frame;
//    }
//    
//    if (self.initFrame.origin.y -  self.scrollView.frame.origin.y < 44 ) {
...
//    }
    ...
    if(self.state != SVPullToRefreshStateLoading) {
        ...
        {
            if (!(self.scrollView.pullToRefreshView.state == SVPullToRefreshStateLoading || self.scrollView.pullToLoadMoreView.state == SVPullToRefreshStateLoading)) {
...
- (void)setState:(SVPullToRefreshState)newState {
    ...
    switch (newState) {
        ...
        case SVPullToRefreshStateLoading:
            ...
            //    [self performSelector:@selector(refreshOver) withObject:nil afterDelay:1.0f];
                [self performSelector:@selector(refreshOver) withObject:Nil afterDelay:0.3f];
