...
@Controller
@RequestMapping("/comm")
public class CollectPostAction {
	...
	@RequestMapping(value="/collectpost")
	@ResponseBody 
	public Result test(HttpServletRequest request,CollectPostDto dto){
		Result result=new Result();
		
		try {
			validParam(dto,result);
			if(result.getSuccess()){
				this.postservice.collectPost(dto,result);
			}else{//验证失败
			}
			return result;
		} catch (Exception e) {
	...
	//表单验证
	public void validParam(CollectPostDto dto,Result result){
