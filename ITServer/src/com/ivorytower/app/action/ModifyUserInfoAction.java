...
public class ModifyUserInfoAction {
	...
	@ResponseBody 
	public Result test(HttpServletRequest request,ModifyUserInfoDto dto){
		Result result=new Result();
		
		try {
			validParam(dto,result);
			if(result.getSuccess()){
				this.userservice.modifyUserInfo(dto,result);
			}else{//验证失败
			}
			return result;
		} catch (Exception e) {
	...
	//表单验证
	public void validParam(ModifyUserInfoDto dto,Result result){
