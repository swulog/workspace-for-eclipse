...
public class GetPhone4GAction {
	
	@Autowired
	private BusService busService;
	
	static Logger logger = Logger.getLogger(GetPhone4GAction.class.getName());
	
	
	@RequestMapping(value="/getPhone4G")
	@ResponseBody 
	public synchronized Result getPhone(HttpServletRequest request,GetPhoneDto getPhoneDto){
		...
		try{
			...
			if(result.getSuccess()){
				...
				this.busService.requestPhone4G(push_appkey,push_masterSecret,push_timeToLive,getPhoneDto,result);
