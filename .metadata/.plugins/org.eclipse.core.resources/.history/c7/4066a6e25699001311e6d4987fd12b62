...
@Controller
@RequestMapping("/comm")
public class QryPostDetailAction {
...
static Logger logger = Logger.getLogger(TestAction.class.getName());
	...
	@RequestMapping(value="/querypostdetail")
	@ResponseBody 
	public Result test(HttpServletRequest request,QryPostDetailDto dto){
		Result result=new Result();
		
		try {
			validParam(dto,result);
			if(result.getSuccess()){
				this.postservice.queryPostDetail(dto,result);
			}else{//验证失败
			}
			return result;
		} catch (Exception e) {
	...
	//表单验证
	public void validParam(QryPostDetailDto dto,Result result){
