...
-(void)initLayout{
    ...
    UIBarButtonItem *item4 = [[[UIBarButtonItem alloc] initWithTitle:@"完成" style:UIBarButtonItemStyleBordered handler:^(id sender) { 
    ...
    switch (style) {
        ...
        case CInputAssistViewCancelAndDone:
            ...
            break;  
        case CInputAssistViewPerviousAndNextAndDone:
            items =  [NSArray arrayWithObjects:btnPervious,btnNext,flexibleSpace,btnDone,nil]; 
            break;
            
        case CInputAssistViewAll:
            items =  [NSArray arrayWithObjects:btnPervious,btnNext,flexibleSpace,btnCancel,btnDone,nil];   
            break; 
        case CInputAssistViewPerviousAndDone:

            items =  [NSArray arrayWithObjects:btnPervious,flexibleSpace,btnDone,nil];
            break;
            
        //wsong update
        case CInputAssistViewPerDoneHide:
            [btnCancel setTitle:@"隐藏"];

            items = [NSArray arrayWithObjects:btnPervious,flexibleSpace,btnCancel,btnDone,nil];
            break;
        case CInputAssistViewNextHide:
            [btnCancel setTitle:@"隐藏"];

                items = [NSArray arrayWithObjects:btnNext,flexibleSpace,btnCancel,nil];
            break;
        case CInputAssistViewPerNextHide:
            [btnCancel setTitle:@"隐藏"];

            items = [NSArray arrayWithObjects:btnPervious,btnNext,flexibleSpace,btnCancel,nil];
            break;
        default:
            break;
    }
